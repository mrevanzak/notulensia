...
import { useGetUserInfo } from "@/lib/api/user/get-user-info-detail";
...
function AppMenuProfile(): ReactElement {
  ...
  return (
    <div className="layout-menu-profile border-none tw-relative overflow-visible tw-z-10 !tw-bg-transparent">
      <Tooltip content={tooltipValue("Profile")!} target=".avatar-button" />
      <button
        className="avatar-button p-link shadow-none pl-8 !tw-rounded-none"
        onClick={toggleMenu}
        type="button"
      >
        <Image
        ...
        <span className="!tw-ml-8">
          <h3 className="tw-text-white">{data?.name}</h3>
          <p className="tw-text-xs tw-text-white">{data?.phoneNumber}</p>
        </span>
        <i
          className={classNames(
            "layout-menu-profile-toggler pi pi-fw tw-text-white tw-font-bold tw-text-xl",
            {
              "pi-angle-down":
      ...
      <ul
        className={classNames(
          "menu-transition tw-border-2 tw-border-[#334798] tw-rounded-2xl ml-8 mr-4 tw-absolute tw-inset-x-0 tw-bottom-20 tw-bg-white",
          {
